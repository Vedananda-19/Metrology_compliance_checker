...
def run(facts_raw: dict, on_date: date | None = None, include_drafts=False):
    ruleset = json.loads((ROOT / "rules" / "lmpc_2011_rules.json").read_text())
    tables = json.loads((ROOT / "rules" / "reference_tables.json").read_text())
    if on_date is None:
